...
description: >
  ...
  graph-paper grid, Noto Sans JP serif 400 + Noto Sans JP + DM Mono, the top/bottom cobalt
...
colors:
  ...
  ink: "#0B0D12"
  ink-soft: "#f0b400"
  grid: "rgba(240, 180, 0, 0.10)"
  ink-faint: "rgba(240, 180, 0, 0.18)"

typography:
  # — reading ramp —
  body:        { fontFamily: "Noto Sans JP", cqw: 0.83, weight: 400, lineHeight: 1.5 }
  body-lede:   { fontFamily: "Noto Sans JP", cqw: 0.95, weight: 400, lineHeight: 1.5 }
  micro:       { fontFamily: "Noto Sans JP", px: 13, weight: 600, tracking: "0.16em", upper: true }
  micro-strong:{ fontFamily: "Noto Sans JP", px: 16, weight: 600, tracking: "0.18em", upper: true }
  ...
  # — display / hero ramp (Noto Sans JP 400, negative tracking) —
  table-name:  { fontFamily: "Noto Sans JP", cqw: 1.5, weight: 400, lineHeight: 1.15 }
  row-headline:{ fontFamily: "Noto Sans JP", cqw: 2.1, weight: 400, lineHeight: 1.05 }
  ed-callout:  { fontFamily: "Noto Sans JP", cqw: 2.6, weight: 400, lineHeight: 1.1, italic: true }
  headline:    { fontFamily: "Noto Sans JP", cqw: 4.6, weight: 400, lineHeight: 0.95 }
  headline-index:{ fontFamily: "Noto Sans JP", cqw: 5.0, weight: 400, lineHeight: 0.95 }
  display-quote:{ fontFamily: "Noto Sans JP", cqw: 5.7, weight: 400, lineHeight: 1.05, tracking: "-0.005em" }
  display-chapter:{ fontFamily: "Noto Sans JP", cqw: 6.8, weight: 400, lineHeight: 1.0, tracking: "-0.005em" }
  display-closing:{ fontFamily: "Noto Sans JP", cqw: 9.4, weight: 400, lineHeight: 0.96, tracking: "-0.005em" }
  display-hero:{ fontFamily: "Noto Sans JP", cqw: 10.4, weight: 400, lineHeight: 0.9, tracking: "-0.008em" }
  vbig-numeral:{ fontFamily: "Noto Sans JP", cqw: 12.5, weight: 400, lineHeight: 0.9, tracking: "-0.015em" }
...
components:
  ...
  ledger-row:
    layout: "grid: mono num · Noto Sans JP name · Hanken desc · mono delta"
...
- **Fonts** — already set to **Noto Sans JP** (display) / **Noto Sans JP** (body); ignore any preset font name lingering in prose.
...
The voice is a three-face conversation: **Noto Sans JP** serif at weight 400 (size, not weight,
makes hierarchy) carries every display and headline; **Noto Sans JP** carries body and
...
- **Noto Sans JP 400** for all display (negative tracking); **Hanken 600** uppercase 0.16em labels; **DM Mono** chrome.
...
- **Squint** — one Noto Sans JP element dominates at **3–6× its nearest neighbor** (`display-hero`/`vbig-numeral`); hierarchy is size, never weight.
...
Hanken→Noto Sans JP, from a mono delta arrow, or from opacity, never from color.

## Typography

Two ramps. The **reading ramp** (Hanken body 0.83cqw, mono chrome in px) carries copy + labels;
the **display/hero ramp** (Noto Sans JP `headline` 4.6cqw → `vbig-numeral` 12.5cqw) carries every
...
- **Noto Sans JP at 400 only**, in cobalt, with negative tracking (hero −0.008em, numeral −0.015em). **Hanken labels uppercase, 0.16–0.18em.** **DM Mono 0.04–0.08em.** No bold serif.
...
**Ground** paper + grid, hairlines. **Composes** pixel-glitch (right, ~26cqw), qr-block (top-right), display-hero, ed-callout. **Focal** a 1–2 line `display-hero` Noto Sans JP headline in cobalt, left-anchored, with an italic `ed-callout` subtitle in `{colors.ink-soft}`. **Chrome** Hanken kicker; mono meta top + page number. **Accent** none (cobalt is the only ink). **Silence** ~45% paper. **Fixed** Noto Sans JP 400, glitch + QR present, grid + hairlines. **Free** title, glitch step pattern, QR placement. **Density** sparse.

### 2 · Index Ledger (catalog · move: dense matrix · left — the dense frame)

**Ground** paper + grid, hairlines, `pad-top`/`pad-bottom`. **Composes** topbar-rule, ledger-rows. **Focal** a topbar (`headline-index` + mono lab-tag, 1.5px rule) over 4–6 ledger rows (mono num · Noto Sans JP name · Hanken desc), ink-faint dividers. **Chrome** page number. **Accent** mono delta arrows. **Silence** tight — the density exception (the grid wants filling). **Fixed** 1.5px topbar rule, 1px ink-faint dividers, Noto Sans JP names. **Free** rows, lab-tag, deltas. **Density** dense-exception.

### 3 · Chapter Opener (section · move: scale · sparse · left)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left, ~14cqw, low opacity), mono index, display-chapter, body-lede. **Focal** a `display-chapter` Noto Sans JP title, with a small mono `CHAPTER NN` index above and an optional Hanken lede ≤42cqw. **Accent** none. **Silence** ~60% — let the grid breathe. **Fixed** Noto Sans JP 400, glitch low-opacity, grid showing. **Free** title, lede, glitch side. **Density** sparse.
...
**Ground** paper + grid, hairlines. **Composes** display-quote (or display-manifesto), attribution rule, optional compact glitch. **Focal** a 2–3 line Noto Sans JP pull in cobalt, with a Hanken kicker above and a 1px cobalt attribution rule + mono byline beneath. **Accent** none. **Silence** ~55% — deliberately open. **Fixed** Noto Sans JP 400, attribution rule, grid showing. **Free** quote, byline. **Density** sparse.

### 6 · Colophon (closer · move: right-aligned close · sparse)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left edge, mirroring the cover), display-closing, mono credit columns. **Focal** a right-aligned `display-closing` Noto Sans JP title with a Hanken kicker above. **Chrome** 3–4 column mono credit grid at the foot; page number. **Accent** none. **Silence** ~50%. **Fixed** glitch on left, Noto Sans JP 400. **Free** closing line, credits. **Density** sparse.
...
- Set **Noto Sans JP at 400 in cobalt**; make hierarchy with size, not weight; negative-track display.
...
- Don't bold Noto Sans JP, round any corner, or add a drop shadow (QR outset is an anti-shadow only).
...
- **Squint** — one Noto Sans JP element dominates at 3–5× its neighbor.
...
- **Type** — Noto Sans JP 400 negative-tracked, fit-to-measure; Hanken labels 0.16em; ≥1.4cqw floor.
...
- **Three Google Fonts** (Noto Sans JP, Noto Sans JP, DM Mono); CJK pairing (Noto Serif SC 700/400) carries over from the source.
